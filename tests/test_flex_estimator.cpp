...
flex::FlexSettings makeObservedSettings(){
  ...
  observed_settings.left_damping = 2 * observed_settings.left_stiffness.cwiseSqrt();
  observed_settings.right_damping = 2 * observed_settings.right_stiffness.cwiseSqrt();
...
//   tau(0) = 55; tau(3) = 55;
//   postureEstimator.correctEstimatedDeflections(tau, q, dq, Eigen::Array3i(0, 1, 2), Eigen::Array3i(3, 4, 5));

//   BOOST_CHECK_EQUAL(q, zero_q);
//   BOOST_CHECK_EQUAL(dq, zero_dq);
//   postureEstimator.resetLeftFlex0(); postureEstimator.resetRightFlex0();
//   q = flex::eVectorX::Zero(6); dq = flex::eVectorX::Zero(6); tau = flex::eVectorX::Zero(6);
